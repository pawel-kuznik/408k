...
module.exports = class extends Entity {
    ...
    set model(value) {
        ...
        // is the value an model ID? then fetch the model from the root
        if (typeof(value) == 'string')  value = this.root.fetchModel(value);

        // store the model ID or NULL if no model
        this[data].model = value ? value.id : null;
    ...
    set replacementModel(model) {
        ...
        // is the value an model ID? then fetch the model from the root
        if (typeof(model) == 'string') model = this.root.fetchModel(model);
